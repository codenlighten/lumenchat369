...
/**
 * Save memory to disk (atomic write for data integrity)
 */
async function saveMemory(memory) {
  ...
  try {
    ...
    await fs.writeFile(tempFile, JSON.stringify(memory, null, 2), 'utf-8');
...
export async function getMemoryContextString() {
  const memory = await loadMemory();
  let context = '';
  const MAX_SUMMARY_LENGTH = 500; // chars per summary
  const MAX_INTERACTION_LENGTH = 400; // chars per interaction
  ...
  if (memory.summaries.length > 0) {
    ...
    memory.summaries.forEach((summary, i) => {
      ...
      const truncatedSummary = summary.text.length > MAX_SUMMARY_LENGTH 
  ...
  // Add recent interactions with content limits
  if (memory.interactions.length > 0) {
    ...
    memory.interactions.forEach(interaction => {
      const rawQuery = interaction.userRequest?.query || 'Request data';
      const userQuery = rawQuery.length > MAX_QUERY_LENGTH
        ? rawQuery.substring(0, MAX_QUERY_LENGTH) + '...'
        : rawQuery;
        
      const aiChoice = interaction.aiResponse?.choice || 'response';
      const rawContent = interaction.aiResponse?.response || 
                        interaction.aiResponse?.code || 
                        interaction.aiResponse?.summary || 
                        'Response data';
      const aiContent = rawContent.length > MAX_CONTENT_LENGTH
